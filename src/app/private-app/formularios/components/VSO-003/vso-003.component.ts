...
export class VSO003Component {
  ...
  public buscarEstudiante(): void {
    this.privateAppService.obtener(`formularios/informacionVSO003/${this.formularioVSO003.get('informacion_estudiante.cedula')!.value}`).subscribe(res => {
      const practicaPreProfesional: PracticaPreProfesional = res.data;
      let notaOrganizacion = null;
      if (practicaPreProfesional.grades && practicaPreProfesional.grades?.length > 0) {
        notaOrganizacion = practicaPreProfesional.grades[0].nota_promedio;
      } else {
        this.appService.alertaAviso('AVISO', 'La organización aún no ha evaluado al estudiante');
      }
  ...
  public calcularNotaPromedio(): void {
    const calificaciones: {id: number, nombre: string, calificacion: number}[] = this.formularioVSO003.get('calificacion.criterios')?.value;
    let total = 0;
    let promedio = 0;
    ...
    promedio = (promedio + this.formularioVSO003.get('calificacion.nota_organizacion')?.value)/2;
  ...
  private obtenerCriteriosCalificacion(): void {
    ...
    this.privateAppService.obtener('catalogos?nombre=CRITERIOS CALIFFICACIÓN').subscribe(res => {
      ...
      const criterioFormArray = this.tranformarAbstractControlEnFormArray(this.formularioVSO003.get('calificacion.criterios')!);
